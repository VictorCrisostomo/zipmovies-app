  ...
  return (
    <main>
      ...
      {movieList.map((filme, key)=>(
        <BannerScreen key={key} filme = {filme.items.filmes[48]} />
      ))}
      {movieList.map((filme, key)=>(
        <BtnsActions key={key} filme = {filme.items.filmes[48]} />
      ))}
      <br />
      <br />
      <CardsFilmes/>
...
export default VelozeseFuriosos2