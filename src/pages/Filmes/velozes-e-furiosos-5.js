  ...
  return (
    <main>
      ...
      {movieList.map((filme, key)=>(
        <BannerScreen key={key} filme = {filme.items.filmes[51]} />
      ))}
      {movieList.map((filme, key)=>(
        <BtnsActions key={key} filme = {filme.items.filmes[51]} />
      ))}
      <br />
      <br />
      <CardsFilmes/>
...
export default VelozeseFuriosos5